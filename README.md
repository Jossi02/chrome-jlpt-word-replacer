...
- **바이브코딩 특강 해커톤** — 핵심 구현은 2026년 8월 19일 하루(약 13시간), 3인 팀으로 진행
- Chrome Manifest V3, **런타임 네트워크 호출 0건**(번역 API·LLM 미사용). 사전 646개를 번들로 싣습니다
- 자동 테스트 39개와 최소 CI, 사전 빌드가 불변식 8종을 강제
- 당시 3사이트 수동 실측으로 오탐 4건을 찾아 제거
...
npm test        # 39/39. 의존성 설치 없이 Node만 있으면 됩니다
```
...
밀도 슬라이더에 `Math.random()`을 쓰면 새로고침마다 화면이 달라져 시연을 못 합니다. 페이지 전체를
관통하는 러닝 카운터와 누적 선택 quota를 비교해, 같은 입력은 항상 같은 후보를 고르면서 0~100%의
실제 비율에 수렴하게 했습니다. 이 카운터를 텍스트 노드마다 초기화하지 않는 계약도 테스트로 고정했습니다.
...
자동 검증은 현재 코드에서 재현한 결과이고, 아래 사이트 실측은 2026년 8월 19일 개발 PC에서 수행한
역사적 수동 검증입니다.

| 항목 | 범위 | 결과 |
| --- | ---: | ---: |
| 자동 테스트 | `node --test` 39건 | 39 통과 / 0 실패 |
| 당시 수동 실측 | 데모 · 위키백과 · 뉴스 기사 3곳 | 수정 후 표본에서 명백한 오치환 0건 |
| 당시 안전성 회귀 | 3사이트 × 6항목 | 17 통과 / 1 해당없음 |
| 당시 복원 검사 | 3사이트 각 3회 | restore 경로에서 원본 텍스트와 일치 |
| 당시 콘솔 검사 | 3사이트 각 재로드 3회 | 0건 |
...
있습니다. 당시 사용한 외부 뉴스 기사 HTML snapshot은 공개 hardening에서 current tree에서 제거했으며,
과거 검증 기록만 남겼습니다.
...
- 함정 케이스를 내장한 자체 시연 페이지와 당시 브라우저 검증 기록
- 파일 단위 협업 하네스의 통합·유지보수 — 현재 공용 설정은 문법 검사와 테스트·리뷰 게이트만 사용
...
- **SPA의 본문 루트 교체는 지원 범위 밖입니다.** 선택된 본문 아래에 추가되는 노드는 관찰하지만,
  애플리케이션이 본문 루트 자체를 교체하면 확장을 다시 실행해야 할 수 있습니다.

## 함께 만든 사람

| | 담당 | 맡은 일 |
| --- | --- | --- |
| [@kimminje2](https://github.com/kimminje2) | 레인 A · 로직 | 최장 일치 매처, 조사·경계 규칙, 밀도 선택, 사전·매처 테스트, 초기 Claude 협업 하네스와 아이콘 |
| [@hersmen98](https://github.com/hersmen98) | 레인 B · DOM | 범위 판정과 제외 규칙, ruby 치환·무손실 복원, 툴팁, 스캔 오케스트레이션 |
| [@Jossi02](https://github.com/Jossi02) | 레인 C · 데이터·셸·UI (통합자) | 사전 파이프라인, manifest/package, 팝업·CSS, 데이터·시연·문서, 실측과 통합·유지보수 |

해커톤 당시에는 `src/`를 세 사람이 나눠 파일 단위 ownership과 레인 branch를 사용했고 feature 작업을
주로 PR로 통합했습니다. 이후 통합·문서·유지보수에는 default branch의 직접 commit도 포함되므로,
[`OWNERS.md`](OWNERS.md)는 현재 강제 정책이 아니라 당시 구현 ownership 기록으로 봐야 합니다.
